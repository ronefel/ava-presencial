...
/**
 * Rone Santos
 * Verifica se o usuário tem a permissão informada no curso.
 * Esta função foi criada para verificar se o usuário é professor da disciplina
 * Usada em 'overview.php' e 'block_completion_progress.php'
 */
function block_user_has_role_in_course($user_id, $course_id, $role = 'student') {
    global $DB;
    
    $sql = "SELECT ra.id
              FROM {role_assignments} AS ra
         LEFT JOIN {user_enrolments} AS ue
                ON ra.userid = ue.userid
         LEFT JOIN {role} AS r
                ON ra.roleid = r.id
         LEFT JOIN {context} AS c
                ON c.id = ra.contextid
         LEFT JOIN {enrol} AS e
