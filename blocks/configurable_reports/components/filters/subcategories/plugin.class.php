...
class plugin_subcategories extends plugin_base {
    ...
    public function execute($finalelements, $data) {
        ...
        if (!$filtersubcategories) {
            ...
            /**
             * Rone Santos
             * Força o ID da categoria do curso da disciplina
             */
            if (preg_match("/%%FILTER_SUBCATEGORIES:([^%]+)%%/i", $finalelements, $output) && $this->report->courseid !== 1) {
                $replace = ' AND '.$output[1].' LIKE CONCAT( \'%/\', '.$this->getCourseCategory($this->report->courseid).', \'/%\' ) ';
    ...
    public function print_filter(&$mform) {
         ...
         * Força o ID da categoria do curso da disciplina
         */
        if(!$filtersubcategories && $reportclass->currentcourseid !== 1){
            $filtersubcategories = $this->getCourseCategory($reportclass->currentcourseid);
        }
        ...
        $mform->getElement('filter_subcategories')->setSelected($filtersubcategories); // Rone Santos, Força o ID da categoria do curso da disciplina
    }

    /**
     * Rone Santos
     * Retorna o ID da categoria do curso que a disciplina está contida
     * 
     * Exemplo: / Graduação / GESTÃO PÚBLICA / EAD2020.1A / 03 / TEORIA POLÍTICA [GES - EAD2020.1A]
     * TEORIA POLÍTICA está contida no curso GESTÃO PÚBLICA então retorna o ID da categoria do curso GESTÃO PÚBLICA
     */
    public function getCourseCategory($courseid) {
        global $DB;

        $sql = "SELECT (string_to_array(cc.path, '/'))[3] AS category
                  FROM mdl_course c
                  JOIN mdl_course_categories cc
                    ON cc.id = c.category
                 WHERE c.id = ?";

        return ($DB->get_record_sql($sql, [$courseid]))->category;
    }
}
