            ...
            if ($rs = $this->execute_query($sql)) {
                foreach ($rs as $row) {
                    ...
                    foreach ($arrayrow as $ii => $cell) {
                        ...
                        /**
                         * Rone Santos
                         * Resolve um bug que acontece quando adiciona uma url com '?'
                         * Esse bug só ocorre quando o usuário não é administrador
                         */
                        if (strpos($cell, '%5B%5BQUESTIONMARK%5D%5D')) {
                            $arrayrow[$ii] = str_replace('%5B%5BQUESTIONMARK%5D%5D', '?', $cell);
                        }
