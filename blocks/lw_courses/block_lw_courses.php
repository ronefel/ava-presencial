...
class block_lw_courses extends block_base {
    ...
    public function get_content() {
        ...
        if (empty($sortedcourses)) {
            ...
            /**
             * Rone Santos - 01/10/2019
             * Mostra outra msg quando o usuário não está inscrito em nenhuma disciplina
             */
            $coursesfromuser = enrol_get_all_users_courses($USER->id);
            if(count($coursesfromuser) > 0){
                $this->content->text .= 'Nenhum professor liberou a visualização da(s) disciplina(s).<br/>No momento você está inscrito em:<br/>';
                foreach ($coursesfromuser as $course) {
                    $this->content->text .= '<strong>' .$course->fullname. '</strong> (oculta para estudantes)<br/>';
                }
                //$this->content->text .= get_string('nocourses', 'my');
            }else{
                $this->content->text .= html_writer::div('<h5><p>No momento você não possui inscrição em curso presencial. Talvez tenha acessado o endereço errado.<br/>Se este for o caso acesse o endereço do AVA EAD: <a href="http://ead.facimed.edu.br">http://ead.facimed.edu.br</a> ou mande uma mensagem para o suporte no chat abaixo.</p></h5>', 'no-available-courses');
            }
