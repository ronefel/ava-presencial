...
/**
 * Upgrade script for format_facimedead
 *
 * @param int $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_format_facimedead_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2017020200) {
        ...
        upgrade_plugin_savepoint(true, 2017020200, 'format', 'facimedead');
    }
    ...
    if ($oldversion < 2017050300) {
        ...
        $sql = "SELECT c.id, c.enddate, cfo.id as cfoid
             ...
             LEFT JOIN {course_format_options} cfo
                 ...
                 WHERE c.format = :format
                   AND cfo.id IS NULL";
        $params = ['optionname' => 'automaticenddate', 'format' => 'facimedead'];
        $courses = $DB->get_recordset_sql($sql, $params);
        foreach ($courses as $course) {
            $option = new stdClass();
            $option->courseid = $course->id;
            $option->format = 'facimedead';
            $option->sectionid = 0;
            ...
            if (empty($course->enddate)) {
                ...
                format_facimedead::update_end_date($course->id);
            } else {
                $option->value = 0;
                $DB->insert_record('course_format_options', $option);
            }
        ...
        upgrade_plugin_savepoint(true, 2017050300, 'format', 'facimedead');
    }
