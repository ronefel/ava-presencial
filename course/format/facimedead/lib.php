 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_facimedead extends format_base {
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_facimedead');
        } else {
            $formatoptions = course_get_format($this->get_course())->get_format_options($section);

            if($formatoptions['parent'] != false){ // Se não pai
                //nome padrão será um número
                return 'Nova Subseção';
            }
            ...
            return get_string('sectionname', 'format_facimedead'). ' '. $section->section. ' - (' . $weekday.' - '.$endweekday.')';
        }
    }
...
//            )
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit = array(
                ...
                'automaticenddate' => array(
                    'label' => new lang_string('automaticenddate', 'format_facimedead'),
                    'help' => 'automaticenddate',
                    'help_component' => 'format_facimedead',
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        // Re-order things.
        //$mform->insertElementBefore($mform->removeElement('automaticenddate', false), 'idnumber'); //esse elemento não existe
     ...
     *
     * In case if course format was changed to 'facimedead', we try to copy options
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_facimedead');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_facimedead', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        $renderer = $PAGE->get_renderer('format_facimedead');
    ...
    public static function update_end_date($courseid) {
        ...
        // section. This query will also validate that the course is indeed in 'facimedead' format.
        ...
        $course = $DB->get_record_sql($sql,
            ['optionname' => 'automaticenddate', 'format' => 'facimedead', 'courseid' => $courseid]);
        ...
        $format = new format_facimedead('facimedead', $courseid);
    ...
    public function course_content_header(){
        $renderable = new format_facimedead_header();
        return $renderable;
    }
    ...
    public function page_set_course(moodle_page $page){
        ...
        if ($this->on_course_view_page()) {
            ...
            if(has_capability('moodle/course:update', $context)){
                ...
                if ($action) {
                    if ($action == 'addsubsection') {
                        $parent = optional_param('parentsection', 0, PARAM_INT);
                        if ($parent) {
                            $sectionnum = $this->create_new_section($parent);
                            //Redirecionar para apagar os parâmetros da URL, setar anchor para seção pai
                            $url = course_get_url($this->courseid, $parent);
                            $url->param('newsubsection',$sectionnum );
                            redirect($url);
                        }
                    }
                }
    ...
    public function create_new_section($parent = 0, $before = null) {
        ...
        foreach ($sections as $sectid => $sect) {
            if ($sectid == $sectionnum) {
                $sectionid = $sect->id;
            }
        }

        foreach ($sections as $subsection) {
            if($subsection->section == $parent){
                $this->update_section_format_options(array('id' => $sectionid, 'parent' => $subsection->section));
            }
        }
        return $sectionnum;
    }
 ...
 * @return \core\output\inplace_editable
 */
function format_facimedead_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'facimedead'), MUST_EXIST);
        return course_get_format($section->course)->inplace_editable_update_section_name($section, $itemtype, $newvalue);
    }
}

/**
 * Class format_buttons_header
 * Ello Oliveira 10/11/2017
 * Um renderable para renderização do título do curso. Recebe a variável global do curso
 */
class format_facimedead_header implements renderable {
    public $course;
