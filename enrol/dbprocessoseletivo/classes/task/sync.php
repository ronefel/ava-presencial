...
namespace enrol_dbprocessoseletivo\task;
...
class sync extends \core\task\scheduled_task
{
    ...
    {
        return "Sincronização de Processos Seletivos";
    }
    ...
    public function execute()
    {
        ...
        if (!enrol_is_enabled('dbprocessoseletivo')) {
            echo('enrol_dbprocessoseletivo plugin is disabled, sync is disabled'."\n");
            exit(1);
        }

        $verbose = !empty($options['verbose']);
        $enrol = enrol_get_plugin('dbprocessoseletivo');
        $result = 0;
