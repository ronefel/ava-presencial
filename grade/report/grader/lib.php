    ...
    private function setup_sortitemid() {
        ...
        if ($this->sortitemid) {
            ...
            if (isset($SESSION->gradeuserreport->sortitemid)) {
                ...
                $this->sortitemid = 'firstname'; // Rone Santos - Ordena pelo primeiro nome
            }
    ...
    public function get_right_rows($displayaverages) {
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                /**
                 * Rone Santos - 18/10/2019
                 * Mostra um ícone quando a nota estiver travada para edição
                 */
                if($item->locked > '0' && $item->locked < time()){
                    $nome = $item->itemname != "" ? $item->itemname : $item->idnumber;
                    $itemcell->text .= $OUTPUT->pix_icon('i/risk_xss', $nome.' travada em: '. userdate($item->locked)).' ';
                }
