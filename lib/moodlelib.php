...
function make_grades_menu($gradingtype) {
    ...
    if ($gradingtype < 0) {
        ...
        /**
         * Rone Santos
         * Se a nota máxima for maior que 0 e menor ou igual a 2, monta a grade com ponto flutuante
        ...
        if($gradingtype > 0 && $gradingtype <= 2) {
            $maxgrade = $gradingtype * 10;
            for ($i=$maxgrade; $i>=0; $i--) {
                $index = $i/10;
                $maxgradestr = number_format($maxgrade/10, 2 , ',', '');
                $grades[(string)$index] = number_format($i/10, 2 , ',', '') .' / '. $maxgradestr;
            }
        } else {
