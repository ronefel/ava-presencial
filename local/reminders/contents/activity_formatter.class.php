    ...
    public function append_info(&$htmlmail, $modulename, $activity, $user=null, $event=null) {
        if (isset($activity->timeopen)) {
            ...
            if ($utime > $activity->timeopen) {
                 ...
                 *  Adicionado css para melhorar a visualização
                 */
                $htmlmail .= html_writer::start_tag('tr', ['style'=> "vertical-align: top;"]);
                $htmlmail .= html_writer::tag('td', get_string('contentdescription', 'local_reminders'), ['style'=> "padding-top: 21px;"]);
