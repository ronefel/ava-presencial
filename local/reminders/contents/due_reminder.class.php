...
class due_reminder extends course_reminder {
    ...
    public function get_message_html($user=null) {
        ...
        $htmlmail .= html_writer::start_tag('body', array('id' => 'email', 'style'=> "max-width:1000px;margin-left:auto;margin-right:auto"));
        $htmlmail .= html_writer::start_tag('div', array('id' => 'redor'));
        $htmlmail .= html_writer::start_tag('div', array('id' => 'redor2', 'style' => 'border-radius:30px;background-color:rgba(256,256,256,0.95);text-align: justify;',));
        
        $htmlmail .= html_writer::start_tag('img', array('src' =>'http://portal.facimed.edu.br/Corpore.Net/facimed/imagens/logo.png'));
        
        if(isset($user->firstname)){
            $htmlmail .= html_writer::tag('p', "Olá ".$user->firstname.',');
        }

        $htmlmail .= html_writer::start_tag('div', array('id' => 'info', 'style'=>'color:black'));

        $custom_msg = '';
        if($this->event->eventtype == 'due'){
            $custom_msg .= html_writer::tag('p', "Esta mensagem é para lembrá-lo que faltam menos de $this->aheaddays dia(s) da entrega da atividade <strong>". $this->cm->get_context_name() . "</strong>, do curso <strong>".$this->course->fullname."</strong>." );
            $custom_msg .= html_writer::tag('p', "A atividade deve ser enviada até <strong>". $this->format_event_time_duration($user) . "</strong>." );
            $custom_msg .= html_writer::tag('p', "Se você já submeteu esta tarefa, por favor desconsidere esta mensagem.");
        }
        else if($this->event->eventtype == 'open'){
            $custom_msg .= html_writer::tag('p', "Esta mensagem é para avisá-lo que faltam menos de  $this->aheaddays dia(s) da abertura da atividade <strong>". $this->cm->get_context_name() . "</strong>, do curso <strong>".$this->course->fullname."</strong>." );
            $custom_msg .= html_writer::tag('p', "A atividade se inicia em <strong>". $this->format_event_time_duration($user) . "</strong>." );
            $custom_msg .= html_writer::tag('p', "Se você já concluiu a atividade, por favor desconsidere esta mensagem.");
        }
        else if($this->event->eventtype == 'close'){
            $custom_msg .= html_writer::tag('p', "Esta mensagem é para avisá-lo faltam menos de $this->aheaddays dia(s) do encerramento da atividade <strong>". $this->cm->get_context_name() . "</strong>, do curso <strong>".$this->course->fullname."</strong>." );
            $custom_msg .= html_writer::tag('p', "A atividade deve ser enviada até <strong>". $this->format_event_time_duration($user) . "</strong>." );
            $custom_msg .= html_writer::tag('p', "Se você já concluiu a atividade, por favor desconsidere esta mensagem.");
        }
        else{
            $custom_msg .= html_writer::tag('p', "Esta mensagem é para lembrá-lo do prazo de entrega da atividade <strong>". $this->cm->get_context_name() . "</strong>, do curso <strong>".$this->course->fullname."</strong>." );
            $custom_msg .= html_writer::tag('p', "A atividade deve ser enviada até ". $this->format_event_time_duration($user) . "." );
            $custom_msg .= html_writer::tag('p', "Se você já submeteu esta tarefa, por favor desconsidere esta mensagem.");
        }

        $htmlmail .= $custom_msg;
        $htmlmail .= html_writer::end_tag('div');
