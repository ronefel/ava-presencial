...
class helper {
    ...
    public static function get_member_info(int $referenceuserid, array $userids, bool $includecontactrequests = false,
        ...
        foreach ($otherusers as $member) {
             ...
             * Lista as disciplinas que o usuário está inscrito
             */
            $data->courses = self::get_enrolled_courses($member->id);
    ...
    * Lista as disciplinas que o usuário está inscrito
    */
    private static function get_enrolled_courses($id){
        $courses = enrol_get_all_users_courses($id);

        $coursesFormated = [];
        foreach ($courses as $course) {
            $coursesFormated[] = ['name' => $course->fullname];
        }
