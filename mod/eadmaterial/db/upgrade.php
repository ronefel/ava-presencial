...
function xmldb_eadmaterial_upgrade($oldversion=0) {
...
/// for demonstrative purposes and to show how the eadmaterial
/// iself has been upgraded.

/// For each upgrade block, the file eadmaterial/version.php
...
/// First example, some fields were added to the module on 20070400
    ...
    if ($result && $oldversion < 2019013103) {

    /// Define field course to be added to eadmaterial

        if(!$dbman->table_exists('eadmaterial_log')){
            $dbman->install_one_table_from_xmldb_file($CFG->dirroot . '/mod/eadmaterial/db/install.xml',
            'eadmaterial_log');

            upgrade_mod_savepoint(true, 2019013103, 'eadmaterial');
        }
    ...
    /// Define field intro to be added to eadmaterial
        $table = new XMLDBTable('eadmaterial');
    ...
    /// Define field introformat to be added to eadmaterial
        $table = new XMLDBTable('eadmaterial');
    ...
    if ($result && $oldversion < 2019013103){
        if($dbman->table_exists('eadmaterial')){

            if($dbman->field_exists('eadmaterial', 'name')){
                $DB->execute("UPDATE {eadmaterial} SET name = materialtitle");
...
/// "01" in the last two digits of the version
    ...
    /// Define field timecreated to be added to eadmaterial
        $table = new XMLDBTable('eadmaterial');
    ...
    /// Define field timemodified to be added to eadmaterial
        $table = new XMLDBTable('eadmaterial');
    ...
    /// Define index course (not unique) to be added to eadmaterial
        $table = new XMLDBTable('eadmaterial');
...
/// Third example, the next day, 20070402 (with the trailing 00), some inserts were performed, related with the module
    ...
    /// Add some actions to get them properly displayed in the logs
        $rec = new stdClass;
        $rec->module = 'eadmaterial';
        $rec->action = 'add';
        $rec->mtable = 'eadmaterial';
        $rec->filed  = 'name';
    ...
    if($result && $oldversion < 2018011500){
        $table = new XMLDBTable('eadmaterial');
        $field = new XMLDBField('introformat');
