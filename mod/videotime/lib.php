...
function videotime_supports($feature) {
    switch ($feature) {
        case FEATURE_GRADE_HAS_GRADE:         return false; // Rone Santos
...
function videotime_cm_info_dynamic(cm_info $cm) {
    ...
    // $sessions = \videotimeplugin_pro\module_sessions::get($cm->id, $USER->id); // Rone Santos
    ...
    if ($instance->label_mode == 1) {
        ...
        /**
         * Rone Santos
         * Removendo chamadas para o objeto #sessions
         */
        ...
        $PAGE->requires->js_call_amd('mod_videotime/videotime', 'init', [NULL, 5, videotime_has_pro(),
