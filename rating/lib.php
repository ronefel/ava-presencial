    ...
    public function get_aggregate_string() {
        ...
        if (is_numeric($aggregate) && $method != RATING_AGGREGATE_COUNT) {
            if ($method != RATING_AGGREGATE_SUM && !$this->settings->scale->isnumeric) {
                ...
                /**
                 * Rone Santos
                 * Se a nota máxima for menor ou igual a 2, monta a nota com ponto flutuante
                 * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
                 * Essa function é chamada quando abre uma postagem em um fórum
                 */
                if ($this->scaleid > 0 && $this->scaleid <= 2) {                    
                    $aggregatestr .= number_format($aggregate/10, 2 , ',', '');
                } else {
...
class rating_manager {
    ...
    public function get_all_ratings_for_item($options) {
        ...
        /**
         * Rone Santos
         * Se a nota máxima for maior que zero e menor ou igual a 2 divide a avaliação por 10
         * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
         * Essa function é chamada quando clica na avaliação de um comentário em um fórum
         */
        $sql = "SELECT r.id, CASE WHEN (r.scaleid > 0 AND r.scaleid <= 2) THEN r.rating::FLOAT / 10 ELSE r.rating END AS rating, r.itemid, r.userid, r.timemodified, r.component, r.ratingarea, $userfields
                  FROM {rating} r
        ...
        foreach ($options->items as $item) {
            ...
            foreach ($userratings as $userrating) {
                ...
                if ($item->{$itemidcol} == $userrating->itemid) {
                    ...
                    /**
                     * Rone Santos
                     * Se a nota máxima for maior que zero e menor ou igual a 2 força o retorno da nota da avaliação.
                     * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
                     * Essa function é chamada quando clica na postagem em um fórum
                     */
                    if($ratingoptions->scaleid > 0 && $ratingoptions->scaleid <= 2){
                        $ratingoptions->rating = $userrating->usersrating;
                    }else{
            ...
            if (array_key_exists($item->{$itemidcol}, $aggregateratings)) {
                ...
                /**
                 * Rone Santos
                 * Se a nota máxima for maior que zero e menor ou igual a 2 força o retorno da nota de agregação da avaliação.
                 * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
                 * Essa function é chamada quando clica na postagem em um fórum
                 */
                if($ratingoptions->scaleid > 0 && $ratingoptions->scaleid <= 2){
                    $ratingoptions->aggregate = $rec->aggrrating;
                }else{
    ...
    protected function generate_rating_scale_object($scaleid) {
        ...
        if (!array_key_exists('s'.$scaleid, $this->scales)) {
            ...
            if ($scaleid < 0) {
                ...
                /**
                 * Rone Santos
                 * Se a nota máxima for maior que zero e menor ou igual a 2 monta a nota com ponto flutuante.
                 * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
                 * Essa function é chamada quando clica na postagem em um fórum ela monta o campo select de avaliação do fórum.
                 */
                if ($scaleid > 0 && $scaleid <= 2) {
                    $scaleid_modified = $scaleid * 10;
                    for ($i = 0; $i <= (int)$scaleid_modified; $i++) {
                        $scale->scaleitems[(string)$i] = number_format($i/10, 2 , ',', '');
                    }
                } else {
        ...
        if ($results) {
            ...
            foreach ($results as $rid => $result) {
                if ($options->scaleid >= 0) {
                    ...
                    /**
                     * Rone Santos
                     * Se a nota máxima for maior que zero e menor ou igual a 2 divide a nota da avaliação por 10.
                     * Isso foi feito para avaliar o fórum com notas decimais 0,2 ou 0,7
                     * Essa function é chamada quando seleciona uma nota de avaliação no fórum.
                     */
                    if($options->scaleid > 0 && $options->scaleid <= 2) {
                        $result->rawgrade = $result->rawgrade / 10;
                    }
    ...
    public function add_rating($cm, $context, $component, $ratingarea, $itemid, $scaleid, $userrating, $rateduserid,
        ...
        if (!$pluginpermissionsarray['rate']) {
             ...
             * Essa function é chamada quando seleciona uma nota de avaliação no fórum.
             */
            if ($scaleid > 0 && $scaleid <= 2 && $userrating != RATING_UNSET_RATING) {
                $params['rating'] = $userrating / 10;
            }
