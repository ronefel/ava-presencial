...
if (!$isSessionMobile) {$standardlayout = TRUE;} // Rone Santos - 21/11/2019
$sidebar = FALSE;
if ($isSessionMobile) {$sidebar = TRUE;} // Rone Santos - 21/11/2019
    ...
    <div id="page-content" class="row-fluid">
        <div id="<?php echo $regionbsid ?>" class="span12">
            <div class="row-fluid">
                ...
                <section id="region-main" class="span9 pull-right"> <!-- Rone Santos --> 
                <?php } else { ?>
                <section id="region-main" class="span12"> <!-- Rone Santos -->
                <?php } ?>
                    <?php
					$body_attributes = $OUTPUT->body_attributes();
	     			if (strpos($body_attributes, 'pagelayout-mypublic') !== false) {echo $OUTPUT->full_header();}
                    echo $OUTPUT->course_content_header();
                    echo $OUTPUT->blocks('content-pre', ''); //content-pre é nescessário Rone
                ...
                <?php 
				if ($standardlayout) {echo $OUTPUT->blocks('side-pre', 'span3 desktop-first-column pull-left');} // Rone Santos
				else if (!$sidebar) {echo $OUTPUT->blocks('side-pre', 'span3 desktop-first-column pull-right');} // Rone Santos
                ?>                
                <?php echo $OUTPUT->blocks('side-post', 'span3'); ?> <!-- Rone Santos -->
            </div>
        </div>
    </div>
