 ...
 */
 ...
 class theme_lambda_core_renderer extends core_renderer {
    ...
    public function standard_head_html() {
        global $SITE, $PAGE, $CFG; // Rone Santos
        ...
        /**
         * Rone Santos
         * Adicionado script para detectar o dispositivo do usuário
         * */
        $output .= "<script type='text/javascript' src='$CFG->wwwroot/theme/lambda/mobile-detect.min.js'></script>";
    ...
    /**
     * Rone Santos
     * Adiciona o chat do tawk.to nas seguintes páginas:
     * - página inicial;
     * - painel;
     * - página principal do curso
     * 
     * Quando for acesso por dipositivo mobile:
     * - Esconde o botão padrão
     * - Adiciona um botão personalizado mais pequeno
     * 
     * @return string HTML fragment.
     */
    public function standard_end_of_body_html() {
        ...
        if ( $PAGE->url->get_host() == 'ava.facimed.edu.br' &&
            ($PAGE->pagelayout == 'frontpage' ||
             $PAGE->pagelayout == 'mydashboard' ||
             $PAGE->pagelayout == 'course') ) {
                $output .= "<!--Start of Tawk.to Script-->
                            <script type='text/javascript'>
                            var Tawk_API=Tawk_API||{}, Tawk_LoadStart=new Date();
                            (function(){
                            var s1=document.createElement('script'),s0=document.getElementsByTagName('script')[0];
                            s1.async=true;
                            s1.src='https://embed.tawk.to/5d14cc5553d10a56bd7c2849/default';
                            s1.charset='UTF-8';
                ...
                $output .= "<script type='text/javascript' src='$CFG->wwwroot/theme/lambda/tawk-personalizado.js?v=04052020'></script>";
        }
        return $output;
    }
	...
	public function custom_menu($custommenuitems = '') {
        ...
        // Rone Santos - 02/09/2019
        // Monta o menu customizado de acordo com usuário estar logado ou não
        if (isloggedin()){ // Se logado
            $custommenuCustom = $this->getCustomMenuCustom($custommenuitems, 'logado');
        }
        if (!isloggedin()){ // Se deslogado
            $custommenuCustom = $this->getCustomMenuCustom($custommenuitems, 'deslogado');
        }
        //Esta parte do menu customizado é renderizada se usuário está logado ou deslogado
        $custommenuCustom .= $this->getCustomMenuCustom($custommenuitems, 'qualquer');

        $custommenu = new custom_menu($custommenuCustom, current_language());
        return $this->render_custom_menu($custommenu);
    }

    /**
     * Rone Santos - 02/09/2019
     * Extrai o conteúdo entre as tags informadas
     */
    public function getCustomMenuCustom($custommenuitems, $tag){
        preg_match('/<' .$tag. '>(.*?)<\/' .$tag. '>/s', $custommenuitems, $match);
        $result = $match[0];
	...
	protected function render_custom_menu(custom_menu $menu) {
        ...
        if (isloggedin() && !isguestuser()  && $hasdisplaymycourses) { 
            ...
            // Rone Santos - 05/11/2009
            // Novo link para biblioteca virtual
            $menu->add('Biblioteca', new moodle_url('https://middleware-bv.am4.com.br/SSO/facimed/', array('login'=>$USER->username,'token'=>md5($USER->username.'XDECg9CxZmSAPabJKzs8' )  )), 'Biblioteca Virtual Universitária', 1);

            if (!$sortorder = $CFG->navsortmycoursessort) {
                $sortorder = 'sortorder';
            }
            $courses_limit = $CFG->navcourselimit;
            if ($mycourses = enrol_get_my_courses(['enddate'], 'visible DESC, '.$sortorder.' ASC', $courses_limit)) { // Rone Santos, adiciona enddate na consulta pra condição abaixo ter efeito
                foreach ($mycourses as $mycourse) {

                    /**
                     * Rone Santos
                     * Só mostra as disciplinas em progresso
                     */
                    $classify = course_classify_for_timeline($mycourse);
    ...
    public function edit_button(moodle_url $url) {
        ...
        // Rone Santos - 14/08/2019
        // Adiciona o botão Ativar edição/Desativar edição
        $editurl = new moodle_url('/course/view.php', array('id' => $this->page->course->id));
        ...
        if ($this->page->user_is_editing()) {
            ...
            $editstring = '<i class="fa fa-pencil" style="margin-right:3px"></i> ' . get_string('turneditingon');
        }

        $menuhtml = $OUTPUT->render(new action_link($editurl, $editstring, null, ['class'=>"btn btn-secondary",'style'=>"background-color: #990006;margin-left: 5px;color: #ffffff;"]));

        $menuhtml .= $this->render($menu);
